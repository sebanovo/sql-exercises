...
SELECT CLIENTE.CI, CLIENTE.NOMBRE , COUNT(*) AS CANTIDAD_FACTURA
...
FROM FACTURA, CLIENTE
WHERE CLIENTE.CI = FACTURA.CICLIENTE
GROUP BY FACTURA.CICLIENTE
HAVING COUNT(FACTURA.NRO) >= 2; -- es una consulta sobre el resultado

-- Mostrar la cantidad total vendida del producto para el paracetamoL 500MG
SELECT SUM(DETALLEFACTURA.CANTIDAD) AS CANTIDAD_TOTAL
FROM DETALLEFACTURA, PRODUCTO
WHERE PRODUCTO.CODIGO = DETALLEFACTURA.CODIGOPROD
AND PRODUCTO.NOMBRE = 'PARACETAMOL 500MG';

-- Mostrar la cantidad total vendida por cada producto
SELECT PRODUCTO.CODIGO, PRODUCTO.NOMBRE, SUM(DETALLEFACTURA.CANTIDAD) AS CANTIDAD_TOTAL
FROM DETALLEFACTURA, PRODUCTO
WHERE PRODUCTO.CODIGO = DETALLEFACTURA.CODIGOPROD
GROUP BY DETALLEFACTURA.CODIGOPROD;

-- Mostrar los productos que se han vendido una cantidad total mayor a 5
SELECT PRODUCTO.CODIGO, PRODUCTO.NOMBRE, SUM(DETALLEFACTURA.CANTIDAD) AS CANTIDAD_TOTAL_MAYOR
FROM DETALLEFACTURA, PRODUCTO
WHERE PRODUCTO.CODIGO = DETALLEFACTURA.CODIGOPROD
GROUP BY DETALLEFACTURA.CODIGOPROD
HAVING SUM(DETALLEFACTURA.CANTIDAD) > 5;

-- Mostrar la cantidad total comprada de AMOXICILINA500MG por la cliente LUCIA VARGAS
SELECT SUM(CANTIDAD) AS CANTIDAD_TOTAL_COMPRADA
...
FROM CATEGORIA CATEGPADRE, CATEGORIA CATEGHIJO, PRODUCTO
WHERE CATEGHIJO.IDCAT = CATEGPADRE.ID
AND (PRODUCTO.IDCAT = CATEGHIJO.ID OR 
PRODUCTO.IDCAT = CATEGPADRE.ID)
AND CATEGPADRE.NOMBRE = 'ANALGESICOS';
...
SELECT PRODUCTO.CODIGO, PRODUCTO.NOMBRE, COUNT(DETALLEFACTURA.CODIGOPROD) AS CANTIDAD_VECES_VENDIDA
FROM DETALLEFACTURA, PRODUCTO
