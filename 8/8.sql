...
(			
	...
	FOREIGN KEY(CILECTOR)
		REFERENCES LECTOR(CI)			
		ON UPDATE CASCADE			
		ON DELETE CASCADE,			
	CODIGOMB INTEGER NOT NULL,			
	FOREIGN KEY(CODIGOMB) 
		REFERENCES MATERIALBIBLIOGRAFICO(CODIGO)			
		ON UPDATE CASCADE			
		ON DELETE CASCADE			
);			
...
SELECT LECTOR.CI, LECTOR.NOMBRE, COUNT(MATERIALBIBLIOGRAFICO.CODIGO) AS CANTIDAD_LIBRO
...
SELECT MATERIALBIBLIOGRAFICO.CODIGO, MATERIALBIBLIOGRAFICO.TITULO, COUNT(FICHAPRESTAMO.CODIGOMB) AS CANTIDAD_SACADO
FROM MATERIALBIBLIOGRAFICO, FICHAPRESTAMO
WHERE MATERIALBIBLIOGRAFICO.CODIGO = FICHAPRESTAMO.CODIGOMB
GROUP BY FICHAPRESTAMO.CODIGOMB
HAVING COUNT(FICHAPRESTAMO.CODIGOMB) > 1;
